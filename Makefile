...
.PHONY: format-import
format-import:
	isort tartiflette_plugin_time_it/. tests/. setup.py

.PHONY: format
format: format-import
	black -l 79 --target-version py36 tartiflette_plugin_time_it tests setup.py

.PHONY: check-import
check-import:
	isort --check-only tartiflette_plugin_time_it/. tests/. setup.py

.PHONY: check-format
check-format:
	black -l 79 --target-version py36 --check tartiflette_plugin_time_it tests setup.py
